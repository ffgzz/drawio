...
import { extractSvgSize, validateSvg } from "../utils/xml.js";
import {
  generateSymbolId,
  nextItemId,
  showStatus,
...
function buildTemplateSpec() {
  ...
  baseSize =
    ...
    deps.extractSvgSize(state.uploadedPrimarySvg);
...
export function updateSelectedItem(type, id) {
  var state = getTemplateEditorRuntime().state;
...
export function updatePreview(spec) {
  var runtime = getTemplateEditorRuntime();
  var deps = runtime.deps;
  var state = runtime.state;
    ...
    state.schemaFields.forEach(function (field) {
      ...
      function syncField(showError) {
        ...
        enumInput.style.borderColor =
          field.type != "enum" || field.enumValues.length > 0 ? "" : "#b3261e";
  ...
  function loadTemplateIntoEditor(template, options) {
    ...
    primaryName.innerText =
      deps.trim(state.uploadedPrimarySvg).length > 0
        ? state.uploadedPrimarySvgName
        : "未选择默认SVG";
    state.schemaFields = deps
      .flattenSchemaFields(spec.schema, "", [])
      .map(function (field) {
  ...
  function restoreDraftIfExists() {
    ...
    try {
      ...
      draftSpec =
        ...
        deps.trim(draft.currentSpec.svg || draft.uploadedPrimarySvg).length > 0
...
export function openEditorWithTemplate(template) {
  var state = getTemplateEditorRuntime().state;
  if (state.window == null) {
    state.window = createWindow();
  }
