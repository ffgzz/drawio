...
import { normalizePortPoint } from "./specPorts.js";
import { normalizeFrameConfig } from "./frameCore.js";

export function makeCabinetRootStyle() {
  return (
...
export function createCabinetBodySvg(descriptor) {
...
export function makeCabinetBodyStyle(descriptor) {
  return (
    "shape=image;image=" +
    "data:image/svg+xml," +
...
export function makeCabinetGapStyle(selected) {
  return (
...
export function normalizeGapRatio(value, fallbackValue) {
  return clamp(toFloat(value, fallbackValue != null ? fallbackValue : 0.12), 0, 1);
}

export function normalizeCabinetPort(raw, index) {
...
export function normalizeCabinetModel(raw) {
  raw = isObject(raw) ? cloneJson(raw) : {};
  var portCount = Math.max(
    2,
    Array.isArray(raw.ports)
      ? raw.ports.length
      : toInt(raw.portCount, ELECTRICAL_CONSTANTS.CABINET_DEFAULT_PORT_COUNT),
  );
  var ports = [];
  var i;
  var gapRatios = [];
...
export function buildCabinetOffsets(cabinetModel, frameConfig) {
  ...
  for (i = 0; i < modelData.ports.length; i++) {
    if (i > 0) {
      ...
      var nextGap =
        i < modelData.gapRatios.length ? modelData.gapRatios[i] * usableHeight : 0;
      var candidateOffset = currentOffset + previousGap;
      var candidatePage = Math.floor(Math.max(0, candidateOffset - 0.0001) / usableHeight);
  ...
  return {
    ...
    offsets,
    totalLogicalHeight:
      (offsets.length > 0 ? offsets[offsets.length - 1] : modelData.tailPadding) +
      modelData.tailPadding,
  };
}

export function getPageIndexForOffset(offset, usableHeight, pageCount) {
  if (pageCount <= 1 || offset <= 0) {
    return 0;
  }

  return clamp(Math.floor((offset - 0.0001) / usableHeight), 0, pageCount - 1);
}
...
export function buildCabinetPageDescriptors(cabinetModel, frameConfig) {
  ...
  for (pageIndex = 0; pageIndex < pageCount; pageIndex++) {
    ...
    for (i = 0; i < layout.cabinetModel.gapRatios.length; i++) {
      ...
      var gapAbsoluteEnd =
        i + 1 < layout.offsets.length ? layout.offsets[i + 1] : gapAbsoluteStart;
