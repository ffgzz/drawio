 ...
 */
// pluginRegistry 是一个插件注册表，维护着插件短名称（key）到对应 JS 文件路径（value）的映射。
// 当用户启用某个插件时，系统通过这里查找并加载对应的脚本文件。
App.pluginRegistry = {
  ...
  electricalSymbols: "plugins/electricalSymbols.js",
};

// publicPlugin 是一个公开插件列表，决定哪些插件会出现在 UI 的"内置插件"下拉选择框中
// 即"扩展/插件管理"对话框里用户能看到并手动启用的插件列表
App.publicPlugin = [
  "ex",
  "tips",
  "svgdata",
  ...
  try {
    ...
    if (window.mxscript != null) {
      ...
      if (urlParams["plugins"] != "0" && urlParams["offline"] != "1") {
        ...
        App.loadPlugins(["electricalSymbols"]);
      ...
      this.editor.addListener(
        ...
        mxUtils.bind(this, function () {
          ...
          if (Editor.fitDiagramOnLoad) {
            var b =
              urlParams["border"] != null ? parseInt(urlParams["border"]) : 10;
          ...
          try {
            if (value.charAt(0) == "{") {
              ...
              if (window.opener != null && window.opener[value] != null) {
                ...
                throw new Error(
                  mxResources.get("invalidCallFnNotFound", [
                    "window.opener." + value,
                  ]),
                );
              }
...
App.prototype.executeCreateObject = function (value, done) {
  try {
    EditorUi.debug("App.executeCreateObject", [this], "value", [value]);
      ...
      this.createFile(
        ...
        mxUtils.bind(this, function () {
          ...
          if (urlParams["grid"] != null) {
            this.editor.graph.setGridEnabled(urlParams["grid"] == "1");
          }
          ...
          window.location.hash =
            "create=" + encodeURIComponent(JSON.stringify(value));
        }),
        true,
        null,
        true,
          ...
          if (name != null && name.length > 0) {
            ...
            if (/(\.pdf)$/i.test(name)) {
              ...
              if (prev == null && mode == App.MODE_DEVICE) {
                ...
                forceDialog ||
                prev != mode
              ) // create a new file also with saveAs even if mode is the same as current
              {
      ...
      if (id.charAt(0) == "L") {
        ...
        if (obj.type == "message") {
          ...
          var createMessageHandler = mxUtils.bind(this, function (evt) {
            EditorUi.debug("EditorUi.createMessageHandler", [this], "evt", [
              evt,
            ]);
